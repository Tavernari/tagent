...
def discover_tagent_files(search_paths: List[str], recursive: bool = True) -> Dict[str, List[str]]:
    """
        ...
        for name, obj in inspect.getmembers(module, inspect.isfunction):
            ...
            if not name.startswith('_'):
                ...
                if len(params) >= 2 and params[0] == 'state' and params[1] == 'args':
                    ...
                    print(f"  ⚠ Skipped {name}: Invalid signature (expected: state, args)")
...
Examples:
  ...
  python -m tagent "Generate report" --search-dir . --model openrouter/gpt-4
    ...
    parser.add_argument('--output', 
                       help='Specific tagent.output.py file path')
    ...
    # Agent configuration
    parser.add_argument('--model', default='openrouter/google/gemini-2.5-pro',
